...
batches = utils.batchify_textfile("mss_bodleian.txt", batch_size=2)
...
for batch in batches:
    ...
    dl = DownloadIIIFManifestTask(
        batch,
        output_directory="test_manifests_bod",
        naming_function=lambda x: "test_bod_"+x.split("/")[-1].split(".")[0], multiprocess=10
    )
    dl.process()
